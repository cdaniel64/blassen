...
#include "i2c_master.h"			//include i2c communication branch
#include "uart.h"				//include uart library branch
#include "apds.h"				//include apds library branch
#define BAUD 9600				//define transmission rate
...
#define DEFAULT_CONFIG1         0x60			// No 12x wait (WTIME)
...
int main(void){
	...
	_delay_ms(2000);		//huge delays
	printf("AT\r\n");
	_delay_ms(5000);		//huge delays
	printf("AT+CWMODE=1\r\n");
	_delay_ms(5000);		//huge delays
	printf("AT+CWJAP=\"Wifi\",\"Password\"\r\n");
	while (1) {
		_delay_ms(5000);	//huge delays so we dont get data loss
		printf("AT+CIPMUX=0\r\n");
		
		_delay_ms(5000);	//more huge delays
		printf("AT+CIPSTART=\"TCP\",\"api.thingspeak.com\",80\r\n");
		//setting up teamspeak
		_delay_ms(5000);
		readColor(&red, &green, &blue);
		printf("AT+CIPSEND=104\r\n");
		//preparing to connect to thingspeak
		printf("GET https://api.thingspeak.com/update?api_key=TX0EL61BIR6SH2GO&field1=%05u&field2=%05u&field3=%05u\r\n", red, green, blue);
		_delay_ms(5000);
	}
}
