 ...
 * GccApplication13.c
 *
 * Created: 4/21/2019 3:21:00 PM
 * Author : cdani
 */ 
#define F_CPU 16000000UL		//initialize CPU
#include <avr/io.h>				//call library avr
#include <util/delay.h>			//call delay library
#include <stdio.h>				//studio library

#define BAUDRATE 9600			//define transfer rate
#define BAUD_PRESCALLER (((F_CPU / (BAUDRATE * 16UL))) - 1)

void read_adc(void);        // Function Declarations not printed
void adc_init(void);
	...
	*/
	...
	while(1)
	{
		...
		*/
		...
		snprintf(outs,sizeof(outs),"%3d\r\n", PWMvalue);  // print the value
		USART_tx_string(outs);
		OCR1A = PWMvalue; 
		_delay_ms(125);        //delay of 125 ms
	}
}
/* INIT ADC */
void adc_init(void)
...
void read_adc(void)
{
	...
	adc_value = adc_value / 4;  // Average samples
}
/* INIT USART (RS-232)  */
void USART_init( unsigned int ubrr )
{
